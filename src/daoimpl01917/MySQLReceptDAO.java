...
public class MySQLReceptDAO implements ReceptDAO{
	...
	public List<ReceptDTO> getReceptList() throws DALException {
		List<ReceptDTO> raavareList = new ArrayList<ReceptDTO>();
		ResultSet rs = Connector.doQuery("SELECT * FROM recept");
		try {
			while(rs.next()) {
				raavareList.add(new ReceptDTO(rs.getInt("recept_id"), rs.getString("recept_navn")));
			}
	...
	public void createRecept(ReceptDTO recept) throws DALException {
		Connector.doUpdate("INSERT INTO recept (recept_navn), VALUES (" + recept.getReceptNavn() + ")");
	}
